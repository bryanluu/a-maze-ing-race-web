...
function buildTestMaze() {
  ...
  `;
    ...
    mockRect(document.querySelector("#v2"), { top: 50, left: 0, width: 50, height: 50 });
    mockRect(document.querySelector("#v3"), { top: 50, left: 50, width: 50, height: 50 });
}
beforeEach(() => {
    Player.svg = "<svg data-cursor></svg>";
});
afterEach(() => {
...
describe("collidesWith", () => {
    test("detects AABB overlap and non-overlap", () => {
        ...
        mockRect(document.querySelector("#player"), { top: 10, left: 10, width: 20, height: 20 });
...
describe("canSee", () => {
    test("reports elements inside and outside the visibility box", () => {
        buildTestMaze();
        const player = new Player({ spawnPoint: 0, vizSize: "Small", memory: false });
        mockRect(document.querySelector("#viz"), { top: 0, left: 0, width: 60, height: 60 });
...
describe("checkVisibility", () => {
    test("toggles visible/seen classes based on the viz box", () => {
        ...
        mockRect(document.querySelector("#viz"), { top: 0, left: 0, width: 40, height: 40 });
        const v0 = document.querySelector("#v0"); // inside viz box
        v0.classList.add("maze-tile");
        const v3 = document.querySelector("#v3"); // outside viz box
